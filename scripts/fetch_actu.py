...
FEEDS = [
    ...
    ("Public Sénat", "https://www.publicsenat.fr/rss.xml", "Institutions", "#C9972B"),
]
...
def fetch_all():
    ...
    for source, url, default_tag, color in FEEDS:
        try:
            ...
            print(f"[warn] échec sur {source} ({url}): {exc}", file=sys.stderr)
            continue

        if feed.bozo and not feed.entries:
            print(f"[warn] flux illisible pour {source}: {feed.bozo_exception}", file=sys.stderr)
            continue
